...
static void fat_merge_dir_clusters(struct fat_diter *target,
                             ...
                             const char *path)
{
        ...
        if (err) {
                if (err != -ENOENT)
                        goto out;
                err = 0;
        } else if (fat_count_dentries(&next.di_cluster)
                   + fat_count_dentries(&diter->di_cluster)
                   <= FAT_DENTRIES_PER_CLUSTER) {
                fat_merge_dir_clusters(diter, &next);
